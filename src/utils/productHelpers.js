...
export function getProductThumbUri(p) {
    if (!p) return '';
    return p.image_thumb_url || p.image || '';
}

export function getProductCardUri(p) {
    if (!p) return '';
    return p.image_full_url || p.image || p.image_thumb_url || '';
}
